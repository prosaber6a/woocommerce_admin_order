    ...
    $('#product-sku-search').on('keyup', () => {
        ...
        if (sku !== '') {
            ...
            for (let i = 0; i < productData.length; i++) {
                ...
                data += `
                <li onclick="itemInTable(${arr.id})" class="hover:bg-blue-500 hover:text-white p-2">
...
function itemInTable(id = -1) {
    ...
    let item = {product_id: obj.id, quantity: 1, price: obj.price, name: obj.name, img: obj.img};
...
function renderTable() {
    ...
    if (orderItems.length > 0) {
        ...
        orderItems.forEach((obj) => {
            ...
            data += `
        <tr data-id="${obj.product_id}" class="border-b-2 border-gray-300 mb-1">
            <td class="flex gap-4 items-center m-2"><img class="h-12 border-0" src="${obj.img}"> <span class="block">${obj.name}</span></td>
            <td class="text-center">
                <input type="number" name="quantity" data-id="${obj.product_id}" onchange="update_quantity(this)" value="${obj.quantity}" class="w-16">
            </td>
            <td class="text-center" data-id="${obj.id}">$ ${obj.price}</td>
            <td class="text-center">
    ...
    `;
        ...
        jQuery('#orderitems_table tbody').html(data);
        calculate();
